...
int main(void) {
	...
	while(1) {
		...
		if(flag){
			...
			datos_en_pantalla(score,best_score,vidas_nave);
		...
		if(dibujar_lista_asteroides(l_rock))
			;//dibujo cada elemento de la lista
		if(dibujar_lista_disparos(l_shot))
			;
		if(nave.vida)
			if((nave_dibujar(&nave))==false)
				break;
