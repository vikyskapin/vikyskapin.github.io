...
nave_t nave_crear()
{
	...
	nave.sp_nave= sprite[identificar_sprite(NAVE)];
	nave.sp_chorro= *sprite[identificar_sprite(CHORRO)];
...
{	
	...
	graficador_ajustar_variables(&nave->posicion_x, &nave->posicion_y);
...
bool nave_dibujar(nave_t *nave)
{
	if(graficador_dibujar(nave->sp_nave->nombre, nave->escala, nave->posicion_x, nave->posicion_y, nave->angulo_rotacion)==true){
		if(nave->potencia > 0)
			return(graficador_dibujar(nave->sp_chorro.nombre, nave->escala, nave->posicion_x, nave->posicion_y, nave->angulo_rotacion));
		else 
			return true;
	}
	else 
